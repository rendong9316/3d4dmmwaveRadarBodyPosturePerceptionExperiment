...
close all;  % 不清除scenario变量(批量模式需要)

%% ======================== 可调参数 ========================
scenario     = 'CCdata_walk_0001';  % 场景名 (批量时由base workspace覆盖)
% 如果 base workspace 有 scenario 变量, 使用它 (批量处理模式)
if evalin('base', 'exist(''scenario'',''var'')')
    scenario = evalin('base', 'scenario');
end
frame_start = 35; frame_end = 55;    % 走动帧(帧40-50信号最强)
USE_MTI = true;                       % MTI抑制DC
SUPPRESS_ZERO_VEL = true;            % 零速抑制
SNR_MIN_DB = 6;

N_ANGLE_PAD  = 128;                % 角度FFT补零
ANGLE_PEAK_RATIO_MIN = 1.8;        % 角度峰均比 (提高过滤噪声)
MAX_DET_PER_FRAME = 15;            % 每帧最大检测点数
RANGE_TARGET_M = 2.4;               % 目标距离(从RT分析得)
RANGE_WINDOW_M = 1.0;               % 距离窗口(+-m)
Z_MIN = 0.3;  Z_MAX = 2.5;         % 高度约束 (m) 过滤地面杂波
X_MAX = 1.0;                        % 方位约束 (m) 收紧
...
% 距离窗 (目标在2.4m附近)
range_mask = (r_axis >= RANGE_TARGET_M - RANGE_WINDOW_M) & ...
             (r_axis <= RANGE_TARGET_M + RANGE_WINDOW_M);
...
tx1_chirps = [2, 5, 8, 11];   % 每设备TX1 (用于32ch方位)
...
for fi = 1:nFrames
    ...
    % --- Step 3: 非相干积累 RD (只TX0+TX1, 8路有效信号) ---
    rdPwr = zeros(nRange, nLoops);
    for k = 1:length(tx0_chirps)
        rdPwr = rdPwr + squeeze(mean(abs(rd_cube{tx0_chirps(k)}).^2, 1));
        rdPwr = rdPwr + squeeze(mean(abs(rd_cube{tx1_chirps(k)}).^2, 1));
    end
    rdPwr = rdPwr / (2 * length(tx0_chirps));
    rdDB = 10 * log10(rdPwr + eps);

    % --- Step 4: 2D CA-CFAR ---
    gr = 4;  gd = 2;
    tr = 8;  td = 4;
    tf_cfar = 5.0;  % CFAR阈值 (降低检测更多运动目标)
    ...
    det(~range_mask, :) = false;   % 距离窗约束: 只检测目标距离附近

    zb = floor(nLoops/2) + 1;
    if SUPPRESS_ZERO_VEL
        det(:, zb-4:zb+4) = false;  % 扩大零速抑制
    end
    ...
    for i = 1:nAfterSNR
        ...
        % ---- 5a: 提取32 RX复数值 (TX0+TX1, 无俯仰, 用于方位) ----
        rx32_az = zeros(2*totalRX, 1);
        for d = 1:nDev
            idx0 = (d-1)*nRX*2 + 1;
            rx32_az(idx0 : idx0+nRX-1) = rd_cube{tx0_chirps(d)}(:, ri, di);
            rx32_az(idx0+nRX : idx0+2*nRX-1) = rd_cube{tx1_chirps(d)}(:, ri, di);
        end

        % ---- 5b: 1D FFT沿32RX -> 方位角谱 ----
        az_spec = fftshift(abs(fft(rx32_az, N_ANGLE_PAD)));
        ...
        % ---- 5c: 俯仰角 (TX2 vs TX0 相位差, 幅度加权) ----
        phase_diffs = zeros(totalRX, 1);
        amp_weights = zeros(totalRX, 1);
        for d = 1:nDev
            idx0 = (d-1)*nRX + 1;
            rx_tx0 = rd_cube{tx0_chirps(d)}(:, ri, di);
            rx_tx2 = rd_cube{tx2_chirps(d)}(:, ri, di);
            phase_diffs(idx0 : idx0+nRX-1) = angle(rx_tx2 .* conj(rx_tx0));
            amp_weights(idx0 : idx0+nRX-1) = abs(rx_tx0);
        end
        amp_weights = amp_weights / (sum(amp_weights) + eps);
        phase_diff = sum(phase_diffs .* amp_weights);

        % h_TX ≈ 0.5λ (TX2与TX0的垂直间距, AWR2243级联实际值)
        % 用1λ会在大仰角(>30°)导致相位折叠, 0.5λ展开到90°
...
if exist(dataFile, 'file')
    ...
    all_sc = loaded.all_sc; all_lb = loaded.all_lb; all_pc = loaded.all_pc;
    nExist = length(all_sc);
else
    all_sc = {}; all_lb = []; all_pc = {}; nExist = 0;
end

% 追加当前场景点云
nNew = nExist + 1;
all_sc{nNew} = scenario;
all_pc{nNew} = allPts(:, 1:4);  % [x, y, z, v]

% 根据场景名确定label
action_types = {'walk','jump','swing','bend','boxing','sit','stand','run','falldown','liedown'};
label = -1;
for a = 1:length(action_types)
    if contains(scenario, action_types{a}), label = a-1; break; end
end
if label < 0
    warning('Unknown action for scenario: %s', scenario);
    label = 0;
end
all_lb(nNew) = label;

save(dataFile, 'all_sc', 'all_lb', 'all_pc', 'action_types', '-v7.3');
fprintf('点云追加到: %s (共%d场景)\n', dataFile, nNew);
close all;
